SELECT
 ...
 --      ,CHARINDEX(' ', REVERSE(Games))-1 AS 'Example 2'
        ,LEFT(Games, CHARINDEX(' ', Games) - 1) AS 'Year' -- Split column to isolate Year, based on space (search for a specific character: in this case: 'space' between the season and yaer (procuraa no atributo season apos o espaço)
        -- left retorna tudo que tá esquerda a partir do numero do caractere escolhido (no caso é o '' - espaço vazio)
...
--        ,[City] -- Commented out as it is not necessary for the analysis
  ...
  -- retorna apenas os resultados onde a 'Season' é summer